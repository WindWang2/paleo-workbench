...
- **ModelRegistry**: Catalog-backed registry of computation models (`catalog/models.py` `Model`/`ModelVersion`) with no ML-framework binding; `find_production_model(capability)` returns None when no production model exists. Inference runs go through `prediction/inference_service.py` (DataRun + DERIVED DataVersion with run linkage, honest `failed` status on error). The only shipped models are the `DemoModelProvider` (`demo_only=True`) and the GR-median `LocalAssetProvider` heuristic (`final_scientific_prediction=False`, `model_type="heuristic"`) — the repo ships NO trained AI model.
