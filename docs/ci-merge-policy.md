...
QGIS renderer path filters now also include `paleo_workbench/ui/workstation/**` and `paleo_workbench/qgis_runtime/**` so 编图 host / runtime loader changes re-run the bridge gate. Slow-family collect floor in `slow-tests.yml` is 15 (this doc previously said 12).
