...
| 16（main） | M5a/b + M4 首片：`scripts/cpp-migration/run-integrated-gate.sh` 一键门禁（configure+build+ctest×2+MALLOC 审计，SDK 路径可环境覆盖）；地震菜单“打开体版本…”（volumeVersionIds 下拉→openVolumeVersion）；B `name_search` 写入从原样改为 ASCII 折叠（Python NFKC+casefold 契约的有界实现，非 ASCII 边界在注释+测试声明），`data.catalog_write` 增断言 | 门禁脚本一键 PASS（47/47 ×2 + MALLOC 14/14） | 通过 | M4 其余、Windows、入口决策 |
| 17（main） | M4 第二片：`CatalogDocument` 增 lineage/staging_leases 读模型（oracle_compare 升为真比较，typical 3+1 行对账）；`export_manifest` 落 catalog.json checkpoint 契约（schema 1 全表、原子写、.bak 轮换、models 透传不丢行），接线 PwbDataStore::commit 与 bootstrap/import；新 `data.manifest_export` | 门禁 48/48 ×2 + MALLOC 14/14 | 通过 | models 领域读模型（fixtures 无数据，待真实使用）、Windows、入口决策 |
| 18（main） | M5 收口：`--self-check` 增 M1-M3 全链（新工程→编辑保存→manifest 存在性→SEG-Y 导入→rms→切片 ok；部署树无 fixture 如实跳过）；`cpp-entry-switch-review.md` 切换评审（建议并行启动器增量曝光，Windows/打包/soak 为硬阻塞） | 门禁 48/48 ×2 + MALLOC 14/14；qgis_smoke_app 含新链 | 通过 | 决策与 Windows/打包在外 |
