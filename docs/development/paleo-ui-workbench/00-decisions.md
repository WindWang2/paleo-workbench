...
## D13 视觉回归阈值（rev2，Phase 7 实测定稿——回归 V5 D8 政策）
- 快照 offscreen 渲染，DPR=1；**结构/状态断言为 gate**（层可见性·不透明度
  复原、控件几何/可见性、确定性部件快照 0 差异）。
- **画布像素 diff 为记录性测量，非 gate**：fallback 后端在独立构造场景的
  首帧时序有竞态（同状态残差实测 ~0.5%，跨运行波动 0.09%↔55.9%），与
  V5 D8「像素 diff 非 gate」的既有裁决一致；洋葱皮渲染正确性由层状态
  gate（E1→(True, 0.30)、E2 不受影响、关闭复原用户原值）+ 既有画布渲染
  测试链保证。全部测量数值记录于 04-visual-qa-verification.md。
