...
## Second global review (2026-09-22 Asia/Shanghai)

Re-scan after fix wave 1:

| Check | Result |
| --- | --- |
| CMake ≥ 3.27 in README | PASS |
| Ribbon "未实现/草案" in design authority + 00-plan | PASS (cleared) |
| CONTEXT dual-track banner | PASS |
| Primary-doc Python-as-production-shell regex | PASS (none) |
| Local links (70) among touched/canonical | PASS (0 broken) |
| Diff vs main docs-only (.md) | PASS |
| `git diff --check` trailing whitespace | fixed in follow-up commit |

### Remaining after fixes (not blocking)

- **P2 residual**: submodule READMEs unverified in this worktree (empty checkouts); deeper well-log/seismic user-path docs still thin (honest pointers exist); older `docs/development/**` ledgers remain SHA-scoped evidence (already classified).
- **P3 residual**: CMake STATUS skip strings still say "Python pages remain the production path" in CMakeLists (code comment, not user docs) — explained in dual-track.md.
- **No open P0 / P1** after wave 1.
