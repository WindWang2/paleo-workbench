...
## 第五轮｜M1-6 容差单位 / M4-3a 比例依赖 / M5-B（落地）
...
## 第七轮｜M5 剩余项——修剪/延伸/圆弧/正多边形（落地）

| 项 | 落地内容 |
|---|---|
| 修剪/延伸线 | `trim_line`（line ∩ boundary，保留内部/外部）/ `extend_line_to_boundary`（端边方向延长到边界第一交点）+ 控制器 `trim_extend_selection`（选集单宏可撤销） |
| 圆弧数字化器 | `ArcCaptureTool`（起点 + 过弧点 + 终点三点定圆弧，32 段采样；退化共线回落折线，永不丢输入） |
| 正多边形数字化器 | `RegularPolygonCaptureTool`（中心 + 半径点 + 边数，默认 6 边） |
