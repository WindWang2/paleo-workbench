...
(schema 2; final tree after review amendments: 1760 renames, zero
deletions, 3 restores-to-active recorded as `REVIEW_AMENDMENT_RESTORED`).
...
| LEGACY_PRODUCT_TEST (`tests/**`, incl. the bootstrap-prepended conftest copy) | 829 |
| SCRATCH (`scratch/**` + `.scratch-*.py` flattened together, one-off runners) | 60 |
...
|---|---|---|
| NATIVE_PRODUCT | 208 | complete wired runtime-reachable C++ replacement exists |
| PARTIAL_NATIVE | 79 | partial per-module attribution; product-level replacement via `pwb-platform` |
| LEGACY_REFERENCE | 390 | no 1:1 native unit; replaced at product granularity by the C++ application |
