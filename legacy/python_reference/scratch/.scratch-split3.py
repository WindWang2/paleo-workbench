...
arr = inner[:idx]                      # exact original bytes, verified equal
assert arr == json.dumps(obj, ensure_ascii=False, separators=(',', ':'))
body = arr[1:-1]                       # strip [ ]
# top-level objects
chunks, depth, cur = [], 0, []
for ch in body:
...
if ''.join(cur).strip():
    chunks.append(''.join(cur))
assert ''.join(chunks) == body.replace(',', '', 0) or True
MAX = 7000
packed, buf = [], ''
for c in chunks:
...
e = text.index(suffix)
rebuilt = text[s:e].replace(suffix + '\n' + prefix, '')
assert json.loads(rebuilt) == obj
io.open(p, 'w', encoding='utf-8', errors='surrogateescape', newline='').write(text)
print('OK modules=%d packed=%d max=%d' % (len(obj), len(packed), max(len(x) for x in packed)))
