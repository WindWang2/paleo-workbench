...
for c in chunks:
    ...
    if len(buf) + len(add) > 7000 and buf:
        packed.append(buf)
        buf = c
    else:
        buf += add
...
# verify ONLY the payload line(s)
back = io.open(p, encoding='utf-8', errors='surrogateescape').read()
blines = back.splitlines(keepends=True)
assert blines[0] == lines[0] and blines[1] == lines[1]
payload = ''.join(blines[2:]).replace(prefix, '').replace(suffix, '')
payload = payload.replace('\n', '').replace('\r', '')
assert payload == inner, 'payload mismatch'
print('OK byte-identical: %d modules / %d lines / max %d chars'
