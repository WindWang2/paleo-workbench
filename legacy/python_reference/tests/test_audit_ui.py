...
def test_map_layer_properties_shows_managed_when_no_provenance(qtbot):
    ...
    # The dialog should show a managed-source placeholder for empty
    # provenance, never a bare "None". The General→Provenance pages were
    # restored to English分区面 (a3e73198), so the placeholder reads
    # "managed" — assert the semantic, not the localization.
...
def test_layer_properties_apply_blocks_invalid_classes_json(qtbot, monkeypatch):
    ...
    assert "nvalid Classes JSON" in dlg.classes_error_label.text()
