"""WellSectionDatum multi-mode vertical alignment policy tests.

Ported from well-log-engine `apps/wellplot-desktop/tests/` (workbench #734):
those copies imported the parent repo and could never run inside the engine
repository, so the coverage lives here, where geoviz and paleo_workbench are
both importable in the full monorepo test matrix.
"""
