...
void Sha256::update(const void* data, std::size_t size) {
    ...
    while (size > 0) {
        const std::size_t take =
            std::min(size, static_cast<std::size_t>(64) - buffered_);
        std::copy(bytes, bytes + take, buffer_.begin() + buffered_);
        buffered_ += take;
        bytes += take;
        size -= take;
        if (buffered_ == 64) {
...
std::string Sha256::hex_digest() {
    ...
    for (int i = 0; i < 8; ++i) {
        length[7 - i] = static_cast<std::uint8_t>((bits >> (8 * i)) & 0xFF);
    }
    // Length bytes must not inflate the counter: restore it.
    total_bits_ = bits;
...
std::string Sha256::of_bytes(std::string_view bytes) {
    Sha256 digest;
    digest.update(bytes);
    return digest.hex_digest();
}

std::optional<std::string> Sha256::of_file(const std::filesystem::path& file,
                                           std::size_t chunk) {
