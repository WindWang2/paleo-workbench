...
RawVolumeReader::RawVolumeReader(std::string path, const Tile3& shape,
    ...
    if (ec) {
        throw TiledInferenceError("raw volume not found: " + impl_->path);
    }
    const auto expected = checked_voxels(shape) * impl_->element_bytes;
    if (size != expected) {
        throw TiledInferenceError(
            "raw volume " + impl_->path + " is " + std::to_string(size)
...
std::vector<float> RawVolumeReader::read_voxel_window(
    ...
    if (!stream) {
        throw TiledInferenceError("raw volume " + impl_->path
                                  + " could not be opened");
    }
    ...
    for (std::size_t a0 = 0; a0 < depth; ++a0) {
        for (std::size_t a1 = 0; a1 < rows; ++a1) {
            ...
            if (!stream) {
                throw TiledInferenceError(
                    "raw volume " + impl_->path + " short read");
            }
...
PreprocessedVolumeReader::PreprocessedVolumeReader(
    ...
    if (nodata_ || !quality_mask_.empty()) {
        ...
        for (int a1 = s1; a1 < e1; ++a1) {
            for (int a2 = s2; a2 < e2; ++a2, ++index) {
                ...
                if (track_mask) {
                    ...
                    if (!invalid && nodata_
                        && static_cast<double>(data[index]) == *nodata_) {
                        invalid = true;
                    }
...
bool load_previous_outputs(const std::string& output_dir,
    ...
    std::error_code ec;
    const fs::path classmap_path = fs::path(output_dir) / "classmap.raw";
    const fs::path probmap_path = fs::path(output_dir) / "probmap.raw";
...
void clear_resume_markers(const std::string& work_root) {
    ...
    std::error_code ec;
    fs::remove_all(fs::path(work_root) / "tiles.done", ec);
}
...
void write_partial_outputs(const std::string& output_dir, const Tile3& shape,
    ...
    const fs::path partial_path =
        fs::path(output_dir) / "prediction_output.partial.json";
...
PredictionPipelineResult run_prediction_pipeline(
    ...
    const bool validity_tracking =
        input.nodata.has_value() || !input.quality_mask_uri.empty();
    const long long bytes_per_voxel = 1 + 2 + (validity_tracking ? 1 : 0);
    const long long budget = options.output_budget_bytes > 0
                                 ? options.output_budget_bytes
                                 : kDefaultOutputBudgetBytes;
    const long long max_voxels = budget / bytes_per_voxel;
    const Json input_errors = validate_prediction_input(
        input, package,
        InputValidationOptions{true, false, false, max_voxels});
    ...
    if (!input.quality_mask_uri.empty()) {
        ...
        std::optional<std::string> bytes = detail::read_file_bytes(
            fs::path(input.quality_mask_uri), &error);
        if (!bytes) {
    ...
    if (work_root.empty()) {
        ...
        work_root = (fs::path(options.output_dir) / "inference.work")
                        .generic_string();
    }
    if (options.resume && !options.output_dir.empty()
    ...
    if (!input.uri.empty()) {
        input_provenance["source_sha256"] =
            pwb::domain::Sha256::of_file(fs::path(input.uri)).value_or("");
    }
    ...
    if (result.stats.cancelled) {
        ...
        if (options.write_outputs && !options.output_dir.empty()) {
            ...
            // it and skip the marked tiles (true resume).
            write_partial_outputs(options.output_dir, input.shape, classes,
                                  classmap, probmap, result.stats,
                                  &result.paths);
    ...
    if (!options.output_dir.empty()) {
        std::error_code ec;
        fs::remove(fs::path(options.output_dir)
    ...
    result.summary = compute_prediction_summary(
        ...
        preprocessed.nodata_voxels(), options.keep_probmap);
    ...
    if (options.write_outputs) {
        ...
        const PredictionOutputPaths paths =
            output_paths_for(options.output_dir);
        descriptor.classmap_path = paths.classmap;
        ...
        const std::span<const std::uint8_t> persisted_mask =
            options.write_mask ? std::span<const std::uint8_t>(
                                     preprocessed.valid_mask())
