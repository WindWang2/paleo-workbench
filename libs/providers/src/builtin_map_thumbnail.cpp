...
public:
    Canvas(std::uint32_t width, std::uint32_t height)
        : width_(width), height_(height), pixels_(width * height * 3u, 0xFF) {}
    ...
    // DDA line, 2px wide for stroke visibility at thumbnail scale.
    void draw_line(double x0, double y0, double x1, double y1, Rgb color, double alpha) {
        const int steps = static_cast<int>(std::max(std::abs(x1 - x0), std::abs(y1 - y0)));
        if (steps == 0) {
        ...
        for (int dy = -radius; dy <= radius; ++dy) {
            for (int dx = -radius; dx <= radius; ++dx) {
                if (dx * dx + dy * dy <= radius * radius) {
                    blend(static_cast<int>(cx) + dx, static_cast<int>(cy) + dy, color, alpha);
                }
            }
        }
    }
    ...
    void fill_polygon_rings(const std::vector<std::vector<std::pair<double, double>>>& rings,
        ...
        const int y_end = std::min(static_cast<int>(height_) - 1,
...
struct WorldToScreen {
    ...
    std::pair<double, double> apply(double x, double y) const {
        ...
        const double sx = span_x != 0.0 ? (x - min_x) / span_x * static_cast<double>(width - 1)
                                        : static_cast<double>(width - 1) / 2.0;
        // North up: screen y grows downwards.
        const double sy = span_y != 0.0 ? (max_y - y) / span_y * static_cast<double>(height - 1)
