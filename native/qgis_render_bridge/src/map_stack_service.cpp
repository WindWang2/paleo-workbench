...
std::string QgisMapStack::layoutExport(const std::string& spec_json,
  ...
  context.order_top_first = [this] { return mirrorTreeOrderTopFirst(); };
  context.resolve_layer = [this](const std::string& key) -> QgsMapLayer* {
    // Bridge mirror resolution order: mirror_by_doc → doc-id scan → QGIS
    // id/name (preserved from the original inline implementation).
    auto map_it = impl_->mirror_by_doc.find(key);
    if (map_it != impl_->mirror_by_doc.end()) {
      if (QgsMapLayer* layer =
              project()->mapLayer(QString::fromStdString(map_it->second))) {
        return layer;
      }
    }
    if (QgsMapLayer* layer = findMapMirrorByDocId(project(), key)) {
      return layer;
    }
    if (QgsMapLayer* layer =
  ...
  return pwb::layout_spec_exec::execute_layout_spec(
      context, spec_json, output_path, format, dpi);
}
