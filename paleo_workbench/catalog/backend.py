"""Reference in-memory backend for :class:`CatalogPort`.

This backend exists ONLY so the workbench can run and the business layer can be
tested for data-lifecycle intent while Kimi's ``feat/data-catalog-core`` is not
yet merged. It is deliberately NOT a second SQLite catalog, NOT a managed
storage engine, and NOT a tag persistence store — it is the thinnest possible
in-memory stand-in.

When Core lands, swap :func:`paleo_workbench.catalog.runtime.get_catalog` to
return a Core-backed adapter. No business-module code changes.
...
class InMemoryCatalog:
    """Minimal reference implementation of :class:`CatalogPort`.

    State is held in plain dicts so it can be serialized/deserialized for test
    round-trips (save → reopen → rebuild). This mirrors, at a tiny scale, what
    the Core's SQLite rebuild does for the UI.
    """
    ...
    def register_input(
        ...
        *,
        ...
        stage = DataStage.EXTERNAL if external else DataStage.RAW
    ...
    def direct_ancestors(self, version_id: str) -> list[DataVersionRef]:
        ...
        Used by display payloads to show direct inputs without the cost of a
        full transitive walk per candidate.
        """
    ...
    def from_dict(cls, data: dict[str, Any]) -> "InMemoryCatalog":
        ...
        # Rebuild the managed-input identity index from RAW versions.
