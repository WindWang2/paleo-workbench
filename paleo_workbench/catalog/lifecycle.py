...
- ``ResourceItem`` (legacy) → RAW/EXTERNAL ``DataVersionRef`` via
...
These helpers degrade gracefully when the catalog is the in-memory fallback and
when legacy ids have no version yet (the old code path still runs).
"""
...
def register_resource_input(
    ...
    *,
    ...
    - Managed (``external=False``) → RAW; the source is treated as immutable.
    - External (``external=True``) → EXTERNAL; the source may go missing.
    ...
    if checksum is None:
        ...
        checksum = sha256_of_file(resource.path)
...
def migrate_project_resources(
    ...
    *,
    ...
    managed resource returns the existing version.
    """
    cat = catalog or get_catalog()
    return [register_resource_input(r, catalog=cat) for r in project.resources]
...
def register_export_run(
    *,
    ...
    checksum = sha256_of_file(artifact.output_path)
...
def register_export_output(
    *,
    ...
    (e.g. ``export_well_canvas``). Returns the registered version.
    """
    cat = catalog or get_catalog()
    ...
    checksum = sha256_of_file(output_path)
    version = cat.register_output(
        run_id=run.run_id,
