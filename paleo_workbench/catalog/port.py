...
The protocol is the contract Kimi's ``feat/data-catalog-core`` is expected to
satisfy. Until it lands, :class:`~paleo_workbench.catalog.backend.InMemoryCatalog`
is the reference backend used for tests and standalone runs.

This module deliberately depends only on the standard library so it can be
imported without the heavyweight catalog stack.
"""
...
class CatalogPort(Protocol):
    ...
    def register_input(
        ...
        *,
        ...
        """Register a user-imported / source asset as RAW (or EXTERNAL).
