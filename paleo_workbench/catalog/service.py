...
class DataCatalogService:
    ...
    def export_manifest(self) -> None:
        ...
        Stale-write guard (#1172): a session whose store advanced past its
        baseline is exactly the state a flush already refused to overwrite
        (CatalogStaleWriteError). Letting close/export resurrect that
        in-memory state through the manifest would bypass the guard — the
        manifest is how a future reopen re-imports data — so the same
        comparison refuses here too.
        """
        with self._lock:
    ...
    def resolve_path(self, version: DataVersion) -> Path:
        ...
        if proj_name in parts:
            ...
            if subpath:
                ...
                if cand.is_file():
                    return cand
        # Unresolvable: return the recorded path unchanged so callers and
        # integrity surface it as MISSING instead of a same-named impostor.
        return raw_path
