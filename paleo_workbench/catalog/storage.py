...
def place_managed_file(
    ...
    *,
    ...
    Reads the source exactly once per pass: streams it to a temp file in the
    target directory while accumulating SHA-256, then fsync + rename into
    place and marks the committed payload read-only. With *register_blob* the
    content-store copy is written from the SAME read — the just-placed
    payload is never re-read to build the blob.
    ...
    if known_sha256 is not None and has_blob(project, known_sha256):
        try:
            if source.stat().st_size == blob_size(project, known_sha256) and (
    ...
    # Content-store copy (register_blob) streams from the same read. The
    # digest is unknown until the stream completes, so the blob temp cannot
    # sit in its shard directory yet; it lives at the blobs/ root (scan_blobs
    # skips non-directory root entries, and a crash orphan here is the same
    # class as the shard-dir ``.blob-*`` leftovers of _place_blob_bytes).
    blob_tmp: str | None = None
    blob_fd = -1
    if register_blob:
        blob_root = blob_dir_for(project)
    ...
    _make_readonly(target)
    if blob_tmp is not None:
        _commit_blob_temp(project, hex_digest, blob_tmp)
    if not keep_source:
        safe_unlink(source)
    project_dir = _project_dir(project)
...
def _commit_blob_temp(project_path: Path, digest: str, blob_tmp: str) -> None:
    """Atomically land a pre-written temp file as ``blobs/xx/<digest>``.

    Same idempotency contract as ``_place_blob_bytes``: an already-present
    blob is left untouched (the content-addressed name guarantees equality)
    and the temp is discarded — a blob that appeared concurrently is never
    overwritten.
    """
    target_dir = blob_dir_for(Path(project_path)) / digest[:2]
