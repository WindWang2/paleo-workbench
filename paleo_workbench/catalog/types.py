...
This package defines the **thinnest possible adapter seam** between the
workbench's business modules (pipeline / prediction / mapping / export / import)
and the canonical Data Catalog Core (``feat/data-catalog-core``).

Scope rules (see ``.planning/2026-08-08-data-lifecycle-integration``):

- These value types describe what the business layer *needs* to express
  (stages, version refs, runs, lineage). They are intentionally minimal.
- The real ``DataAsset`` / ``DataVersion`` / ``DataRun`` / ``Lineage`` domain
  models,SQLite catalog, managed storage, hash store and tag persistence are
  owned by the Core branch. We do NOT re-implement them here.
- ``paleo_workbench/catalog/runtime.py`` resolves which backend implements
  :class:`CatalogPort`. When Core is absent it falls back to a minimal
  in-memory reference backend (enough to test business-integration intent).
  When Core lands, only the backend wiring changes — business modules are
  untouched.

Stage vocabulary is fixed and shared with the UI contract (Gemini's Data
Manager) so the same terms (RAW / DERIVED / INTERMEDIATE / OUTPUT) flow end to
end.
"""
...
class DataStage(str, Enum):
    """Lifecycle stage of a data version.

    ``RAW`` corresponds to the Core's INPUT asset for user-imported source data
    that must remain immutable once registered (managed import). ``EXTERNAL`` is
    the unmanaged variant (linked-by-reference, source may go missing).
    """
    ...
    def is_input_like(self) -> bool:
        return self in (DataStage.RAW, DataStage.EXTERNAL)
...
class DataVersionRef:
    ...
    def __init__(
        ...
        *,
        ...
        self.name = name
        self.stage = DataStage(stage) if isinstance(stage, str) else stage
        self.path = path
