...
class MapRenderBackend(ABC):
    ...
    # 帧底色是否为浅色——画布/导出据此为比例尺、标题等装饰选择墨色，
    # 避免浅色帧上画浅色字。两个内置后端（fallback / qgis）都是浅色帧。
    light_frame_background = True
...
class FallbackMapRenderBackend(MapRenderBackend):
    ...
    def render_to_painter(self, painter: QPainter, width: int, height: int, *, dpi: float | None = None) -> None:
        ...
        including the white map background the screen frame is filled with.
        """
        if not self._initialized:
            self.initialize()
    ...
    def _paint_layer_points(
        ...
        labels = style.labels or (
            TextStyle(field="text", size=10.0, color="#1f2937")
...
def create_map_render_backend(*, prefer_qgis: bool = True) -> MapRenderBackend:
    ...
    Environment variables no longer demote a usable bridge (M5). Callers that
    need the painter path construct ``FallbackMapRenderBackend`` directly or
    pass ``prefer_qgis=False``.
    """
