...
class AICheckAdvisorDialog(QDialog):
    ...
    def __init__(self, bh_report: dict, fault_report: dict, parent=None):
        ...
        # Rule-based suggestions section — derived from the actual report
        # issues above, not hardcoded well/fault names (which were misleading
        # for any project other than the original sample).
        bh_issues = bh_report.get("issues", [])
        fault_issues = fault_report.get("issues", [])
        suggestion_items: list[str] = []
        for iss in bh_issues:
            suggestion_items.append(
                f"<li><b>钻孔 {iss['borehole']}</b>: {iss['message']}</li>"
            )
        for iss in fault_issues:
            suggestion_items.append(
                f"<li><b>断层 {' & '.join(iss['faults'])}</b>: {iss['message']}</li>"
            )
