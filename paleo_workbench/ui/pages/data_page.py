...
class DataPage(QWidget):
    ...
    def _open_curve_interpretation_dialog(self, asset, catalog_version_id: str) -> None:
        """L3 toolbox UI: curve operations → DERIVED version (RAW untouched)."""
        from paleo_workbench.ui.pages.curve_operation_dialog import run_curve_operation_dialog

        service = self._catalog_service()
        if service is None:
            from PySide6.QtWidgets import QMessageBox

            QMessageBox.information(self, "曲线处理", "未打开数据目录。")
            return
        run_curve_operation_dialog(self, service, catalog_version_id)
        self.refresh()
