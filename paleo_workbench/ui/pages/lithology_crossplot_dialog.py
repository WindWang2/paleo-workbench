...
class LithologyCrossplotDialog(QDialog):
    ...
    def __init__(self, analysis_result: dict, parent=None):
        ...
        <h3 style='color: #f1f5f9; border-bottom: 1px solid #475569; padding-bottom: 4px;'>📊 岩性聚类特征统计 (Lithology Cluster Statistics)</h3>
        ...
        _litho_eval = {
            "砂岩": "<span style='color: #34d399; font-weight: bold;'>优质储层 (Sand)</span>",
            "泥岩": "<span style='color: #f87171; font-weight: bold;'>盖层/隔层 (Shale)</span>",
            "石灰岩": "<span style='color: #60a5fa; font-weight: bold;'>致密/碳酸盐岩 (Limestone)</span>",
            "花岗岩": "<span style='color: #fbbf24; font-weight: bold;'>基底结晶岩 (Granite)</span>",
        }

        for lith, stats in clusters.items():
            eval_str = _litho_eval.get(lith, "<span style='color: #cbd5e1;'>未分类</span>")
            html += f"""
            <tr>
                <td style='padding: 8px; border-bottom: 1px solid #334155; font-weight: bold;'>{lith}</td>
                <td style='padding: 8px; text-align: center; border-bottom: 1px solid #334155;'>{stats['count']}</td>
                <td style='padding: 8px; text-align: center; border-bottom: 1px solid #334155;'>{stats['mean_gr']:.1f} ± {stats['std_gr']:.1f}</td>
                <td style='padding: 8px; text-align: center; border-bottom: 1px solid #334155;'>{stats['mean_ai']:.0f} ± {stats['std_ai']:.0f}</td>
                <td style='padding: 8px; text-align: center; border-bottom: 1px solid #334155;'>{eval_str}</td>
            </tr>
            """
        ...
        <h3 style='color: #818cf8; border-bottom: 1px solid #475569; padding-bottom: 4px;'>💡 储层识别与推断结论</h3>
