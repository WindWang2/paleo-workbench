...
void check_stage_switch_and_layout(MainWindow& window) {
    ...
    // Stage 1: prediction context. The seismic/well docks carry no real
    // panel factory in this composition — a stage profile must not
    // present their "(占位页, 待实现)" placeholder as the stage's work
    // surface (#1450): the profile asks for visible, the frame keeps a
    // factoryless dock hidden.
