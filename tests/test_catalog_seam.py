"""Tests for the Data Catalog seam (Protocol + in-memory reference fake).
...
The production backend is the Core-backed ``CoreCatalogAdapter``; these tests
exercise the protocol against the ``tests/fakes`` in-memory fake via
``set_catalog``.
"""
...
from tests.fakes.inmemory_catalog import InMemoryCatalog, _asset_key, sha256_of_file
...
def test_data_stage_unified_vocabulary():
    # One authoritative lifecycle vocabulary (Core enum): exactly four stages.
    assert {s.value for s in DataStage} == {"raw", "derived", "intermediate", "output"}


def test_version_ref_external_input_is_raw_stage():
    # Externality is expressed via the external flag, not a separate stage.
    ref = DataVersionRef(
        asset_id="a",
        version_id="v",
        stage=DataStage.RAW,
    ...
    assert ref.stage is DataStage.RAW
...
def test_external_input_is_not_managed(catalog: InMemoryCatalog):
    ...
    assert v.stage is DataStage.RAW
...
def test_runtime_get_catalog_none_until_injected():
    # No production in-memory fallback: with nothing injected and no env var,
    # the runtime accessor returns None (callers degrade gracefully).
    reset_catalog()
