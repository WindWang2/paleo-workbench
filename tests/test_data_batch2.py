...
def test_well_table_to_arrays_and_df():
    ...
    # Export ok only
    arrays = well_table_to_arrays(table, include_flagged=False)
    ...
    # Export all
    arrays_all = well_table_to_arrays(table, include_flagged=True)
    assert len(arrays_all["names"]) == 3

    # To DataFrame
    df = well_table_to_dataframe(table, include_flagged=True)
    assert len(df) == 3
