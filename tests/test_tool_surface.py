"""V7 tool surface：可用性矩阵（goal §4/§5）单元测试。
...
def _ctx(**overrides) -> ToolContext:
    """默认：phase2、可编辑线图层（物源线）、编辑会话开启、桥原生。"""
    base = dict(
        project_open=True,
        stage=MappingStage.CONSTRAINT_FACTOR.value,
        ...
        capability=QgisCapabilitySnapshot(mode="native"),
        write_granted=False,
    )
...
def test_no_project_disables_everything_except_nothing() -> None:
    ...
    assert "工程" in avail["pan"].reason


def test_reason_single_source_of_truth() -> None:
    """RAW 阻断原因来自角色门禁（block_reason），不另行编造第二套话术。"""
    ctx = _ctx(
        layer=LayerCapabilitySnapshot(
            layer_id="composite:L9",
            name="初始沉积相",
            role=LayerRole.INITIAL_FACIES_SOURCE.value,
            role_label="初始沉积相（原始）",
            kind="polygon",
            editable=False,
            block_reason="图层角色为「初始沉积相（原始）」（RAW/模型结果）——不可直接编辑；请创建 DERIVED 草稿后编辑",
        ),
        editing=False,
    )
    avail = evaluate_tool("toggle_editing", ctx)
    assert not avail.enabled
    assert avail.reason == ctx.layer.block_reason
...
def test_phase1_primary_tools_enabled() -> None:
    ctx = _ctx(
        stage=MappingStage.FACIES_CALIBRATION.value,
        layer=LayerCapabilitySnapshot(
            layer_id="composite:D1",
            name="沉积相解释草稿",
            role=LayerRole.INITIAL_FACIES_DRAFT.value,
            role_label="沉积相解释（草稿）",
            kind="polygon",
            maturity="draft",
            editable=True,
    ...
    assert not avail["split"].enabled  # selected_count=0


def test_phase1_add_line_hidden() -> None:
    ctx = _ctx(stage=MappingStage.FACIES_CALIBRATION.value)
...
def test_phase2_factor_raster_vector_editing_disabled(role: LayerRole) -> None:
    ctx = _ctx(
        layer=LayerCapabilitySnapshot(
            layer_id="composite:F1",
            name=role.label,
            role=role.value,
            role_label=role.label,
            kind="polygon",
            editable=False,
            block_reason="图层角色为「%s」（RAW/模型结果）——不可直接编辑；请创建 DERIVED 草稿后编辑" % role.label,
        ),
        editing=False,
    )
    avail = availability_for_context(ctx)
...
def test_phase3_integrated_editing_and_product_tools() -> None:
    ctx = _ctx(
        stage=MappingStage.INTEGRATED_COMPILATION.value,
        layer=LayerCapabilitySnapshot(
            layer_id="composite:I1",
            name="综合沉积相",
            role=LayerRole.INTEGRATED_FACIES.value,
            role_label="综合沉积相",
            kind="polygon",
            maturity="draft",
            editable=True,
...
def test_split_inputs_override() -> None:
    ...
    ctx = _ctx(selected_count=3, split_inputs_ready=False)
    assert not evaluate_tool("split", ctx).enabled
    ctx2 = _ctx(selected_count=3, split_inputs_ready=True)
...
def test_frozen_layer_blocks_editing_with_reason() -> None:
    ctx = _ctx(
        layer=LayerCapabilitySnapshot(
            ...
            frozen=True,
            editable=False,
            block_reason="当前结果已冻结（FROZEN）——需先解除冻结或另存草稿",
        ),
    )
    avail = availability_for_context(ctx)
...
def test_style_manager_needs_native_capability() -> None:
    ctx = _ctx(capability=QgisCapabilitySnapshot(mode="unavailable", reason="QGIS 桥不可用"))
...
def test_degraded_capability_reason_visible() -> None:
    ctx = _ctx(capability=QgisCapabilitySnapshot(mode="degraded", reason="镜像同步失败 ×2"))
    avail = evaluate_tool("style_manager", ctx)
    assert not avail.enabled
...
def test_no_active_layer_reasons() -> None:
    ctx = _ctx(
        layer=LayerCapabilitySnapshot(layer_id=None),
        has_active_vector_layer=False,
        vector_layer_writable=False,
        editing=False,
    )
    avail = availability_for_context(ctx)
...
def test_missing_layer_degraded_reason() -> None:
    ctx = _ctx(
        layer=LayerCapabilitySnapshot(
            layer_id="composite:M1", name="缺失引用", missing=True, editable=False,
            block_reason="图层源缺失（文件被移动或删除）",
        ),
    )
    avail = availability_for_context(ctx)
    assert not avail["toggle_editing"].enabled
...
def test_tool_context_from_ui_snapshot_roundtrip() -> None:
    ...
    assert ctx.capability.mode == "unavailable"
