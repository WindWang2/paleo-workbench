...
with INC.open("w", encoding="utf-8", newline="\n") as f:
    ...
    # MSVC caps a single string literal at 16380 bytes (C2026); the table
    # is ~52 KB, so emit adjacent raw literals that concatenate (the
    # payload bytes are identical — chunking is invisible to the parser).
    _CHUNK = 12000
    for _start in range(0, len(modules_json), _CHUNK):
        if _start:
            f.write("\n")
        f.write('R"PWB_MODULES_JSON(')
        f.write(modules_json[_start:_start + _CHUNK])
