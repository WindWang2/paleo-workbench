...
// CGM Version 3 **Binary** subset exporter (B1.CGM.1 / ADR 0054).
//
// Self-written — no third-party CGM SDK. Consumes PreparedScene (same model as
// SVG/PDF). This first slice emits one PICTURE with:
//   - metafile / picture delimiter skeleton
//   - integer VDC (0.01 mm units)
//   - curve polylines
//   - track clip rectangles (as closed polylines)
//   - optional restricted TEXT for track titles (Latin/ASCII only)
//
// Not claimed as product support until host menu + consistency tests land
// (B1.CGM.2+). Pattern fills, pagination multi-PICTURE, and transparency
// degradation diagnostics are later slices.
...
public:
  ...
  void polyline(std::span<const std::pair<std::int16_t, std::int16_t>>
                    points) noexcept;
  // Axis-aligned rectangle as closed polyline (4 corners + close to first).
  ...
  void text(std::int16_t x, std::int16_t y, std::string_view s) noexcept;
...
// Maps a PreparedScene into one continuous CGM PICTURE (B1.CGM.1).
